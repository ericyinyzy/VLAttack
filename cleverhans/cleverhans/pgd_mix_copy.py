...
def projected_gradient_descent(
    ...
    if ls==1:
        ...
        while i < nb_iter:
            # adv_x1=copy.deepcopy(adv_x)
            # print(adv_x[0, 0, :10, :1])
            adv_x1, loss = fast_gradient_method(
                model_fn[0],
                adv_x,
                eps_iter,
                norm,
            ...
            optim_perturb_x1=adv_x1-adv_x
            # print(adv_x[0,0,:10,:1])
            loss_list.append(float(loss.detach().cpu().numpy()))
            # adv_x2 = copy.deepcopy(adv_x)
            adv_x2, loss = fast_gradient_method(
                model_fn[1],
                adv_x,
                eps_iter,
                norm,
            ...
            # print(adv_x[0, 0, :10, :1])
            optim_perturb_x2 = adv_x2 - adv_x
            loss_list.append(float(loss.detach().cpu().numpy()))
            adv_x=adv_x+0.5*optim_perturb_x2+0.5*optim_perturb_x1
            # print(adv_x[0, 0, :10, :1])
            # exit()
            eta = adv_x-ori_x
            eta = clip_eta(eta, norm, eps)
