...
class Adv_attack:
    ...
    def black_box_predict(self,image,text):
        answer_ids, topk_ids, topk_probs, = self.black_model(image, text,
                                                             ...
                                                             k_test=128)
        out_v = []
        for ques_id, answer_id in zip(self.batch['question_id'], answer_ids):
            out_v.append({"question_id": int(ques_id.item()), "answer": self.answer_list[answer_id]})
    ...
    def evaluate(
        ...
        for i, batch in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
            ...
            gth=result[0]['answer']
            score, pred = topk_probs[0].max(dim=0) #topk_probs1,128
            ...
            self.text_bank[str(int(batch['question_id'][0]))]=adv_text # you can store the BERTAttack result self.text_bank from here
            if success==1:
