...
class Adv_attack:
    def __init__(self, nlvr_model,pretrain_model,tokenizer,device,correct_idx_list,correct_pred_list,USE_model):
        ...
        self.black_model = copy.deepcopy(nlvr_model)
        ...
        for ii,top_index in enumerate(list_of_index):
            ...
            for substitute in substitutes:
                ...
                if sim>self.cos_sim:
                    ...
                    prediction = self.black_model(self.ori_imgs, [temp_text],targets=gth, train=False)
    ...
    def evaluate(
        ...
        for i, batch in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
            ...
            if targets != pred_class:
               continue
            gth=targets
            score, pred = prediction[0].max(dim=0)  # topk_probs1,128
            tgt_pos = pred
            adv_text, success, sim = self.bert_attack(batch, tgt_pos, score,gth)
            adv_text.extend([success])
            self.text_bank[str(int(index))]=adv_text
            if success==1:
