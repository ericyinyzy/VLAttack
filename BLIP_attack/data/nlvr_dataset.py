...
class nlvr_dataset(Dataset):
    def __init__(self, transform, image_root, ann_root, split):  
        ...
        urls = {'train':'https://storage.googleapis.com/sfr-vision-language-research/datasets/nlvr_train.json',
                ...
                'test':'https://storage.googleapis.com/sfr-vision-language-research/datasets/nlvr_dev.json'}
        filenames = {'train':'nlvr_train.json','val':'nlvr_dev.json','test':'nlvr_dev.json'}
    ...
    def __getitem__(self, index):    
        ...
        if 'left' not in words and 'right' not in words:
            if random.random()<0.5:
                return {'image1': image1, 'image0': image0, 'sentence': sentence, 'label': label}
            else:
                return {'image1': image1, 'image0': image0, 'sentence': sentence, 'label': label}
        else:
            # if random.random()<0.5:
